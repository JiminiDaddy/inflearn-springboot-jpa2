...
public class OrderSimpleApiController {
	...
	public ResultCode<OrderResponseDto> findOrdersV2() {
		List<OrderResponseDto> responseDtos = orderService.findAllV2();
		return new ResultCode<>(responseDtos, responseDtos.size());
	}

	@GetMapping("/api/v3/simple-orders")
	public ResultCode<OrderResponseDto> findOrdersV3() {
		List<OrderResponseDto> responseDtos = orderService.findAllV3();
		return new ResultCode<>(responseDtos, responseDtos.size());
	}

	@GetMapping("/api/v4/simple-orders")
	public ResultCode<OrderSimpleQueryDto> findOrdersV4() {
		List<OrderSimpleQueryDto> responseDtos = orderService.findAllV4();
		return new ResultCode<>(responseDtos, responseDtos.size());
	}
